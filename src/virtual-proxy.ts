...
const handler: ProxyHandler<any> = {
    ...
    apply (target, value, values) {
        ...
        return this.handler.apply(this.__target__, value, values);
      };
};
...
handler.construct = function (target, values, newtarget) {
  ...
  return this.handler.construct(this.__target__, values, newtarget);
};
...
handler.defineProperty = function (target, key, descriptor_argument) {
  ...
  return this.handler.defineProperty(this.__target__, key, descriptor_argument);
};
...
handler.getOwnPropertyDescriptor = function (target, key) {
  ...
  if (descriptor && !descriptor.configurable && !descriptor.writable)
    return descriptor;
  const __descriptor__ = this.handler.getOwnPropertyDescriptor(this.__target__, key);
...
handler.deleteProperty = function (target, key) {
  ...
  if (descriptor && !descriptor.configurable)
    return false;
  return this.handler.deleteProperty(this.__target__, key);
};
...
handler.ownKeys = function (target) {
  if (Reflect_isExtensible(target))
    return this.handler.ownKeys(this.__target__);
  const keys = Reflect_ownKeys(target);
  const __keys__ = this.handler.ownKeys(this.__target__);
...
handler.preventExtensionsHelper = function (target) {
  const keys = this.handler.ownKeys(this.__target__);
  for (let index = 0, length = keys.length; index < length; index++) {
  ...
  Reflect_setPrototypeOf(target, this.handler.getPrototypeOf(this.__target__));
...
handler.isExtensible = function (target) {
  if (Reflect_isExtensible(target)) {
    if (this.handler.isExtensible(this.__target__))
      return true;
...
handler.preventExtensions = function (target) {
  if (Reflect_isExtensible(target)) {
    this.handler.preventExtensions(this.__target__);
    this.preventExtensionsHelper(target);
  }
  return true;
};
...
handler.getPrototypeOf = function (target) {
  if (Reflect_isExtensible)
    return this.handler.getPrototypeOf(this.__target__);
...
handler.setPrototypeOf = function (target, prototype) {
  if (Reflect_isExtensible(target))
    return this.handler.setPrototypeOf(this.__target__, prototype);
...
handler.has = function (target, key) {
  ...
  if (descriptor && !descriptor.configurable)
    return true;
  if (this.handler.getOwnPropertyDescriptor(this.__target__, key))
    return true;
  const prototype = Reflect_isExtensible(target) ? this.handler.getPrototypeOf(this.__target__) : Reflect_getPrototypeOf(target);
...
handler.set = function (target, key, value, receiver) {
  ...
  let __descriptor__ = this.handler.getOwnPropertyDescriptor(this.__target__, key);
  if (!__descriptor__) {
    const prototype = Reflect_isExtensible(target) ? this.handler.getPrototypeOf(this.__target__) : Reflect_getPrototypeOf(target);
    if (prototype)
...
const VirtualProxy = (target, __target__, __handler__) => {
...
export default VirtualProxy

export const Array = (__target__, __handler__) => VirtualProxy([], __target__, __handler__);

export const Object = (__target__, __handler__) => VirtualProxy({}, __target__, __handler__);

export const StrictFunction = (__target__, __handler__) => VirtualProxy(function () { "use strict"; }, __target__, __handler__);

export const Function = (__target__, __handler__) => VirtualProxy(function () {}, __target__, __handler__);

export const Arrow = (__target__, __handler__) => VirtualProxy(() => {}, __target__, __handler__);

export const StrictArrow = (__target__, __handler__) => VirtualProxy(() => { "use strict"; }, __target__, __handler__);